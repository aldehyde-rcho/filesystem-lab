...
typedef struct{
    ...
    int rev_map[RevNum];//恢复块位图
    int rev_num[RevNum];
	int inode_used;//已被使用的i节点数目
	int blk_used;//已被使用的磁盘块数目
    int rev_used;//按栈的方式恢复，指向最新的删除
}SuperBlk;
...
/*指令集合*/
char*	command[]={"fmt","quit","mkdir","rmdir","cd","ls","mk","rm","vim","rev"};
	...
	while(1){
		...
		switch(choice){
			...
			case 8: scanf("%s",name);
					...
					break;
            /*恢复文件*/
            case 9:if(super_blk.rev_used<=0)
                   printf("rev:cannot rev,there is no file\n");
                   else{
                   scanf("%s",name);
                   //rev_dir(super_blk.rev_map[super_blk.rev_used],name);
                   rev_dir(super_blk.rev_map[super_blk.rev_used-1],super_blk.rev_num[super_blk.rev_used-1],name);
                   super_blk.rev_used--;
                   }
                   break;
			default:
...
int format_fs(void)
{
    ...
    super_blk.rev_map[0]=0;
    super_blk.rev_num[0]=0;
    super_blk.rev_used=0;
...
int rev_dir(int pos,int num,char* name)
{
   ...
   // fseek(Disk,InodeBeg+sizeof(Inode)*inode,SEEK_SET);
	...
	for(int i=dir_num-1;i>pos;i--){
		/*pos之后的元素都往前移动一位*/
		dir_table[i]=dir_table[i-1];
	}
	strcpy(dir_table[pos].name,name);
	dir_table[pos].inode_num=num;
	return 1;
}
int adjust_dir(char* name)
{
    ...
    super_blk.rev_map[super_blk.rev_used]=pos;
    super_blk.rev_num[super_blk.rev_used]=dir_table[pos].inode_num;
    super_blk.rev_used++;
